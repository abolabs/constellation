...
class ServiceInstanceAPIController extends AppBaseController
{
    ...
    {
        $serviceInstances = $this->serviceInstanceRepository->all(
            $request->except(['skip', 'limit']),
            $request->get('skip'),
            $request->get('limit')
        );

        return $this->sendResponse(ServiceInstanceResource::collection($serviceInstances), 'Service Instances retrieved successfully');
    }
